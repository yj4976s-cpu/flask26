    ...
    def delete_post(post_id, upload_folder='uploads/'):
        ...
        try:
            with conn.cursor() as cursor:
                ...
                # 3. 게시글 삭제(DB 외래키 ON DELETE CASCADE 설정 덕분에 attachment도 자동으로 삭제)
                # 삭제시 cascade 옵션을 안하면 자식 테이블 데이터를 선 삭제후 부모테이블 데이터 삭제 해야함
    ...
    def update_post(post_id, title, content, files= None, upload_folder='uploads/'):
        ...
        try:
            with conn.cursor() as cursor:
                ...
                # 2. 새 파일들이 들어왔을 경우만 기존파일 삭제 및 교체
                # (아무 파일도 선택 안 하면 기존파일 유지)
