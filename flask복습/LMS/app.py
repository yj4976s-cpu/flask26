...
def board_write():
    ...
    if request.method == 'GET':
        ...
        if 'user_id' not in session:
            return '<script>alert("로그인 후 이용 가능 합니다."); history.back();</script>'
...
def board_delete(board_id):
    ...
    try:
        with conn.cursor() as cursor:
            # 게시글 존재 여부 + 작성자 확인을 위해 DB조회
            sql = "SELECT * FROM boards WHERE id = %s" # 삭제 권한 검증을 위해 데이터베이스에서 id를 가져온다
            cursor.execute(sql, (board_id,))
            row = cursor.fetchone()
            if not row:
                return "<script>alert('존재하지 않는 게시글입니다.'); history.back();</script>"

            # 작성자 id와 맞지않으면 권한없음 메시지 출력
            if row['member_id'] != session.get('user_id'):
            ...
            if cursor.rowcount > 0:
                ...
                return "<script>alert('삭제할 게시글이 없습니다.'); history.back();</script>"
...
def score_members():
    ...
    try:
        with conn.cursor() as cursor:
            sql ="""
                SELECT m.id, m.name, m.uid, s.id AS score_id
                FROM members m
